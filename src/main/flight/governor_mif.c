...
void governorInitModels(void)
{
    ...
    govAutoEnabled  = (governorConfig()->gov_auto_timeout > 0);
...
static void govDebugStats(void)
{
    ...
    DEBUG32F(15, vbValue * 1e3f);
    DEBUG32F(16, vbMean * 1e3f);
    DEBUG32F(17, ibValue * 1e3f);
    DEBUG32F(18, ibMean * 1e3f);

    DEBUG32F(19, govHeadSpeed * 1e3f);
    DEBUG32F(20, hsDiff * 1e3f);
    DEBUG32F(21, hsMean * 1e3f);
    DEBUG32F(22, hsVar * 1e0f);

    DEBUG32F(23, vtValue * 1e3f);
    DEBUG32F(24, vtDiff * 1e3f);
    DEBUG32F(25, vtMean * 1e3f);
    DEBUG32F(26, vtVar * 1e0f);

    DEBUG32F(27, cxValue * 1e6f);
    DEBUG32F(28, cxDiff * 1e6f);
    DEBUG32F(29, cxMean * 1e6f);
    DEBUG32F(30, cxVar * 1e12);

    DEBUG32F(31, ffValue * 1e6f);
    DEBUG32F(32, ffDiff * 1e6f);
    DEBUG32F(33, ffMean * 1e6f);
    DEBUG32F(34, ffVar * 1e6f);

    DEBUG32F(35, hsvtCovar * 1e9f);
    DEBUG32F(36, cxffCovar * 1e9f);

    DEBUG32F(37, ccValue * 1e3f);
    DEBUG32F(38, csValue * 1e6f);
    DEBUG32F(39, cfValue * 1e6f);

    DEBUG32F(40, ccEstimate * 1e3f);
    DEBUG32F(41, csEstimate * 1e6f);
    DEBUG32F(42, cfEstimate * 1e6f);
...
{
    vtValue = cxValue = 0;
    hsMean = vtMean = cxMean = 0;
    hsDiff = vtDiff = cxDiff = 0;
    hsVar = vtVar = cxVar = 0;
    hsvtCovar = cxffCovar = 0;
...
static void govUpdateStats(void)
{
    ...
    // Analysis
    if (govOutput[GOV_MAIN] > 0.10f && govHeadSpeed > 100)
    {
        vtValue = (vbMean - vbOffset) * govOutput[GOV_MAIN];
        cxValue = vtValue / govHeadSpeed;
        
        hsDiff = govHeadSpeed - hsMean;
...
static void govUpdateActiveStats(float govMain)
{
    ...
    if (govMain > 0.25f && govMain < 0.95f &&
        govI > -0.025f && govI < 0.025f &&
        govHeadSpeed > govMaxHeadspeed * 0.1f && govHeadSpeed < govMaxHeadspeed * 1.1f)
    {
        if (ffVar > 0.001f)
...
static void govUpdateEstimate(void)
{
    // Calculate throttle estimate from the physical model
    float estimate = (govSetpointLimited * (cfEstimate * ffValue + csEstimate) + ccEstimate) /
        (vbMean - vbOffset);

    // Set base throttle
    govBaseThrottle = constrainf(estimate, 0, 0.99f);
}
...
void governorUpdateModel2(void)
{
    ...
    // Handle throttle off separately for SAFETY!
    if (throttleStatus == THROTTLE_LOW) {
        if (govState == GS_GOVERNOR_ACTIVE)
            govSaveCalibration();
        govChangeState(GS_THROTTLE_OFF);
        govResetStats();
    }
