...
void governorUpdate(void)
{
    ...
    if (getMotorCount() > 0) {
        ...
        if (govSpooledUp && govSetpoint > 0.0) {
            ...
            float govCollectiveFF = 0; // TODO govColKf * pidGetCollectiveStickPercent();

            // Collective pitch impulse feed-forward for the main motor
            float govCollectivePulseFF = 0; // TODO govColPulseKf * pidGetCollectiveStickHPF();
            ...
            float govCyclicFF = 0; // TODO govCycKf * getCyclicDeflection();
