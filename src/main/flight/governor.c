...
static FAST_RAM_ZERO_INIT float govResistance;
...
void governorInit(void)
{
    ...
    govKp            = (float)governorConfig()->gov_p_gain / 100.0;
    govKi            = (float)governorConfig()->gov_i_gain / 100.0;
    ...
    govResistance    = (float)governorConfig()->gov_resistance / 1000.0f;
...
void governorUpdate(void)
{
    ...
    case GS_GOVERNOR_ACTIVE:
        ...
        if (throttle >= 0.20) {
            ...
            if (govMethod > 0) {
                govBaseThrottle = constrainf((govSetpointLimited * (cfEstimate * ffValue + csEstimate) + ccEstimate) / (vbMean - vbOffset - govResistance*ibMean), 0.20f, 0.99f );
            }
            ...
            if (govMain > 0.25f && govMain < 0.95f && govI > -0.04f && govI < 0.04f && headSpeed > govMaxHeadspeed * 0.1f) {
                if (ffVar > 0.01f)
