...
void governorInit(void)
{
    ...
    govCycKf         = (float)governorConfig()->gov_cyclic_ff_gain / 100.0f;
    govColKf         = (float)governorConfig()->gov_collective_ff_gain / 10000.0f;
    govColPulseKf    = (float)governorConfig()->gov_collective_ff_impulse_gain / 10000.0f;
}


void governorUpdate(void)
{
    float throttle = (float)(rcCommand[THROTTLE] - PWM_RANGE_MIN) / (PWM_RANGE_MAX - PWM_RANGE_MIN);
    ...
    if (getMotorCount() > 0) {
        ...
        else if (!govSetpoint && throttle <= lastSpoolThrottle) {
        ...
        else if (!spooledUp && govSetpoint && (headspeed > govSetpoint*0.97)) {
        ...
        // Case 4: Governor is enabled, not spooled up, over 90% throttle
        else if (!spooledUp && govSetpoint && (lastSpoolThrottle > 0.90f)) {
        ...
        } else if (!govSetpoint && !spooledUp && (lastSpoolThrottle < throttle)) {
        ...
        } else if (govSetpoint && !spooledUp && (headspeed < govSetpoint)) {
        ...
        // --------------- Governor Logic --------------------
        if (spooledUp && govSetpointLimited > 0) {
        ...
        DEBUG_SET(DEBUG_GOVERNOR, 2, getMotorRPM(0));
    ...
    if (getMotorCount() > 1) {
        ...
        float govTail = pidData[FD_YAW].SumLim * MIXER_PID_SCALING;
