...
void mixerInit(void)
{
    ...
    for (int i = 0; i < MIXER_RULE_COUNT; i++) {
        ...
        if (rule->output < MIXER_OUTPUT_MOTORS)
            mixerActiveServos = MAX(mixerActiveServos, rule->output);
        else
            mixerActiveMotors = MAX(mixerActiveMotors, rule->output - MIXER_OUTPUT_MOTORS);
