...
float getMinMotorFreq(void)
{
    float rpm = 1e6;
    for (int i = 0; i < motorCount; i++) {
        rpm = MIN(rpm, getFilteredMotorRPM(i));
    }
    return rpm / 60.0f;
}
...
void mixerInit(mixerMode_e mixerMode)
{
    ...
    //  NOTE:  MixerInit is called from init.c well before pidInit, so don't depend on anything from there (like pidGetDT())
    ...
    // Setup filter for governor base throttle
...
static void calculateThrottleAndCurrentMotorEndpoints(timeUs_t currentTimeUs)
{
...

    {
        ...
        if (idleMinMotorRps > 0.0f) {
            ...
            const float minRps = getMinMotorFreq();
...
static void applyMixToMotors(float motorMix[MAX_SUPPORTED_MOTORS], motorMixer_t *activeMixer)
{
    ...
    if (motorCount > 0) {
        ...
        if (throttle > 0.50 && govMaxHeadspeed > 0) {
            ...
            float rampRate = govRampRate * (float)govMaxHeadspeed;
        ...
        DEBUG_SET(DEBUG_GOVERNOR, 2, getFilteredMotorRPM(0));
        DEBUG_SET(DEBUG_GOVERNOR, 3, getFilteredMotorRPM(1));
