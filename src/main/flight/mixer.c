...
PG_REGISTER_ARRAY(mixer_t, MIXER_RULE_COUNT, mixerRules, PG_GENERIC_MIXER_RULES, 0);
...
void mixerUpdate(void)
{
    ...
    if (FLIGHT_MODE(PASSTHRU_MODE)) {
        ...
        mixerInput[MIXER_IN_STABILIZED_ROLL]  = pidData[FD_ROLL].Sum  * MIXER_PID_SCALING;
        mixerInput[MIXER_IN_STABILIZED_PITCH] = pidData[FD_PITCH].Sum * MIXER_PID_SCALING;
        mixerInput[MIXER_IN_STABILIZED_YAW]   = pidData[FD_YAW].Sum   * MIXER_PID_SCALING;
    }
