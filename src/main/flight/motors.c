...
FAST_RAM_ZERO_INIT float          motorOutput[MAX_SUPPORTED_MOTORS];
FAST_RAM_ZERO_INIT float          motorOutputDisarmed[MAX_SUPPORTED_MOTORS];

FAST_RAM_ZERO_INIT float          motorOutputMin;
FAST_RAM_ZERO_INIT float          motorOutputMax;
...
void initEscEndpoints(void)
{
    ...
    if (isMotorProtocolDshot()) {
        ...
        motorOutputMin  = DSHOT_MIN_THROTTLE;
        motorOutputMax  = DSHOT_MIN_THROTTLE + (DSHOT_MAX_THROTTLE - DSHOT_MIN_THROTTLE) * motorOutputLimit;
    }
    else {
        motorOutputStop = motorConfig()->mincommand;
        motorOutputMin  = motorConfig()->minthrottle;
        motorOutputMax  = motorConfig()->minthrottle + ((motorConfig()->maxthrottle - motorConfig()->minthrottle) * motorOutputLimit);
    }

    motorOutputRange = motorOutputMax - motorOutputMin;
}

void initRpmSource(void)
{
    for (int i = 0; i < MAX_SUPPORTED_MOTORS; i++) { 
        ...
        int freq = constrain(motorConfig()->motorRpmLpf[i], 10, 100);
...
void motorUpdate(void)
{
    if (ARMING_FLAG(ARMED)) {
        for (int i = 0; i < motorCount; i++) {
            ...
            float out = mixerGetMotorOutput(i) * motorOutputRange + motorOutputMin;
            motorOutput[i] = constrainf(out, motorOutputMin, motorOutputMax);
        }
