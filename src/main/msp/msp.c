...
static bool mspProcessOutCommand(int16_t cmdMSP, sbuf_t *dst)
{
    ...
    case MSP_SERVO_CONFIGURATIONS:
        for (int i = 0; i < MAX_SUPPORTED_SERVOS; i++) {
            sbufWriteU16(dst, 0); // servoParams(i)->min);
            sbufWriteU16(dst, 0); // servoParams(i)->max);
            sbufWriteU16(dst, 0); // servoParams(i)->mid);
            sbufWriteU8(dst, 0);  // servoParams(i)->rate);
    ...
    case MSP_SERVO_MIX_RULES:
        for (int i = 0; i < 2*MAX_SUPPORTED_SERVOS; i++) {
    ...
    case MSP_MIXER_CONFIG:
        sbufWriteU8(dst, 23); // was mixerConfig()->mixerMode
    ...
    case MSP_PID_ADVANCED:
        ...
        sbufWriteU8(dst, 0); // HF3D: Antigravity removed
        sbufWriteU8(dst, 0); // was currentPidProfile->d_min[PID_ROLL]
        sbufWriteU8(dst, 0); // was currentPidProfile->d_min[PID_PITCH]
        sbufWriteU8(dst, 0); // currentPidProfile->d_min[PID_YAW]
        sbufWriteU8(dst, 0); // currentPidProfile->d_min_gain
        sbufWriteU8(dst, 0); // currentPidProfile->d_min_advance
...
static mspResult_e mspProcessInCommand(mspDescriptor_t srcDesc, int16_t cmdMSP, sbuf_t *src)
{
    ...
    case MSP_SET_SERVO_CONFIGURATION:
        ...
        if (i >= MAX_SUPPORTED_SERVOS) {
            ...
            sbufReadU16(src);  // servoParamsMutable(i)->min = sbufReadU16(src);
            sbufReadU16(src);  // servoParamsMutable(i)->max = sbufReadU16(src);
            sbufReadU16(src);  // servoParamsMutable(i)->middle = sbufReadU16(src);
            sbufReadU8(src);   // servoParamsMutable(i)->rate = sbufReadU8(src);
    ...
    case MSP_SET_PID_ADVANCED:
        ...
        if (sbufBytesRemaining(src) >= 14) {
            ...
            sbufReadU8(src); // HF3D: Antigravity removed
        }
